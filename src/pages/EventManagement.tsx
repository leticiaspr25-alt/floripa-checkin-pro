...
export default function EventManagement() {
  ...
  const logActivity = async (action: string, details: string) => {
    ...
    await supabase.from('activity_logs').insert({
      event_id: id, user_id: user.id, user_email: user.email, action, details,
    });
  };
  ...
  const handleAddGuest = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('guests').insert({
      event_id: id, name: newGuest.name, company: newGuest.company || null, role: newGuest.role || null,
    });
  ...
  const handleImportExcel = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (evt) => {
      ...
      const guestsToInsert = jsonData.map((row: any) => ({
        event_id: id, name: row.nome || row.name || '', company: row.empresa || row.company || null, role: row.cargo || row.role || null,
      })).filter((g: any) => g.name);
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from('events').update({
      ...
      layout_mode: eventSettings.layout_mode, // Salva o modo escolhido
    }).eq('id', id);
      ...
      <main className="container mx-auto px-4 py-6 print:hidden">
        <Tabs defaultValue="guests" className="space-y-6" onValueChange={(v) => { if(v === 'history') fetchActivityLogs(); }}>
          ...
          <TabsContent value="guests" className="space-y-6 animate-fade-in">
            {/* Lista de Convidados (Mantida igual) */}
              ...
              <form onSubmit={handleSaveSettings} className="space-y-8">
                ...
                <div className="bg-secondary/50 border border-border rounded-xl p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div 
                      className={`cursor-pointer border-2 rounded-xl p-4 transition-all ${eventSettings.layout_mode === 'standard' ? 'border-primary bg-primary/10' : 'border-border bg-card hover:border-primary/50'}`}
                      onClick={() => setEventSettings({...eventSettings, layout_mode: 'standard'})}
                    >
                      <div className="font-bold text-foreground mb-1">Layout Padrão (Automático)</div>
                      <p className="text-xs text-muted-foreground">Você preenche os dados (SSID, Senha, Links) e o sistema gera o design nas telas públicas.</p>
                    </div>
                    <div 
                      className={`cursor-pointer border-2 rounded-xl p-4 transition-all ${eventSettings.layout_mode === 'full_screen' ? 'border-primary bg-primary/10' : 'border-border bg-card hover:border-primary/50'}`}
                      onClick={() => setEventSettings({...eventSettings, layout_mode: 'full_screen'})}
                    >
                      <div className="font-bold text-foreground mb-1">Arte Digital (Tela Cheia)</div>
                      <p className="text-xs text-muted-foreground">Você faz o upload da imagem pronta (1920x1080). O sistema exibe apenas a imagem.</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="bg-card border border-border rounded-xl p-6 shadow-lg">
                    <div className="flex items-center gap-3 border-b border-border pb-4 mb-6">
                      ...
                      <div>
                        <h3 className="text-lg font-bold text-foreground">{eventSettings.layout_mode === 'full_screen' ? 'Arte TV / Tablet' : 'Conexão Wi-Fi'}</h3>
                        <p className="text-xs text-muted-foreground">Exibição nas telas horizontais</p>
                      </div>
                    </div>
                    ...
                    {eventSettings.layout_mode === 'standard' ? (
                      ...
                      <div className="space-y-4 animate-fade-in">
                        <Label className="block mb-3">Upload da Arte Horizontal (1920x1080)</Label>
                        ...
                        <p className="text-xs text-muted-foreground text-center">Esta imagem ocupará 100% da tela na rota /wifi</p>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-card border border-border rounded-xl p-6 shadow-lg">
                    <div className="flex items-center gap-3 border-b border-border pb-4 mb-6">
                      <div className="p-2.5 bg-primary/10 rounded-lg text-primary">{eventSettings.layout_mode === 'full_screen' ? <Monitor size={24} /> : <ExternalLink size={24} />}</div>
                      <div>
                        <h3 className="text-lg font-bold text-foreground">{eventSettings.layout_mode === 'full_screen' ? 'Arte Totem Vertical' : 'Galeria Moments'}</h3>
                        <p className="text-xs text-muted-foreground">Exibição nas telas verticais</p>
                      </div>
                    </div>
                    ...
                    {eventSettings.layout_mode === 'standard' ? (
                      <div className="space-y-4 animate-fade-in">
                        ...
                        <div className="mt-4 pt-4 border-t border-border">
                          ...
                          <div className="flex gap-2 mb-4 p-1 bg-secondary rounded-lg border border-border">
                            <button type="button" onClick={()=>setPhotoMode('auto')} className={`flex-1 py-2 text-xs font-bold rounded ${photoMode==='auto'?'bg-primary text-primary-foreground':'text-muted-foreground hover:text-foreground'}`}>GERAR AUTOMÁTICO</button>
                            <button type="button" onClick={()=>setPhotoMode('upload')} className={`flex-1 py-2 text-xs font-bold rounded ${photoMode==='upload'?'bg-primary text-primary-foreground':'text-muted-foreground hover:text-foreground'}`}>UPLOAD IMAGEM</button>
                          </div>
                      ...
                      <div className="space-y-4 animate-fade-in">
                        <Label className="block mb-3">Upload da Arte Vertical (1080x1920)</Label>
                        ...
                        <p className="text-xs text-muted-foreground text-center">Esta imagem ocupará 100% da tela na rota /totem</p>
                      </div>
                    )}
                  </div>
                </div>
